...
def get_frontend_host(request: Request) -> str:
    """Resolve the public frontend base URL for activation links."""
...
def can_generate_activation_links(user_doc: dict) -> bool:
    if not user_doc:
        return False
    role = user_doc.get("role")
    return role in ("owner", "subaccount") or is_super_admin(user_doc)


@api_router.post("/activation_tokens/generate")
async def generate_activation_token(request: Request):
    """Generate a single-use activation token linked to the calling user."""
    user = await get_user_from_token(request)
    if not user:
        raise HTTPException(status_code=401)
    user_doc = await db.users.find_one({"user_id": user.user_id})
    if not can_generate_activation_links(user_doc):
        raise HTTPException(status_code=403, detail="Vous n'avez pas la permission de générer des liens d'activation")
    ...
    activation_url = f"{get_frontend_host(request)}/activation/{token}"
    return {"token": token, "url": activation_url}


@api_router.get("/activation_tokens")
async def list_activation_tokens(request: Request, scope: Optional[str] = None):
    """List activation links generated by the current user (owners may include filiales with scope=all)."""
    user = await get_user_from_token(request)
    if not user:
        raise HTTPException(status_code=401)
    user_doc = await db.users.find_one({"user_id": user.user_id})
    ...
    cursor = db.activation_tokens.find(q, {"_id": 0}).sort("created_at", -1)
    tokens = await cursor.to_list(length=500)
    base = get_frontend_host(request)
    for t in tokens:
        t["url"] = f"{base}/activation/{t['token']}"
    return tokens
...
async def create_order(
    ...
    snapchat: Optional[str] = Form(None),
...
async def validate_order(order_id: str, request: Request):
    ...
    profile_doc = {
        ...
        "facebook": order.get("facebook"),
        "snapchat": order.get("snapchat"),
        "card_type": "profile",
