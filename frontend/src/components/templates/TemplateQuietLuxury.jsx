...
const TemplateQuietLuxury = ({ profile }) => {
  ...
  const socialLinks = rawCandidates
    ...
    .filter(Boolean)
    .slice(0, 4); // max 4 links displayed
            ...
            <div className="flex gap-2">
              {phones.length > 1 ? (
                <button type="button" onClick={() => setCallMenuOpen(v => !v)}
                  className={`flex-1 bg-white/[0.03] border border-white/10 text-white/90 rounded-xl ${hasPortfolio ? 'py-4' : 'py-5'} text-[10px] tracking-[0.15em] flex items-center justify-center gap-3 active:bg-white/10 transition-all backdrop-blur-sm uppercase`}>
                  <FaPhone className='text-[#C4A77D] text-lg' style={{ transform: 'none' }} /> APPELER
                  <FaChevronDown className={`text-[10px] transition-transform ${callMenuOpen ? 'rotate-180' : ''}`} />
                </button>
              ) : (
